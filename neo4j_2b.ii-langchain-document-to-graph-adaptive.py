    ...
    while retry_count <= max_retries:
        try:
            ...
            if retry_count <= max_retries:
                print(f"  ⚠️ Error processing chunk {chunk_index+1} (attempt {retry_count}/{max_retries}): {e}")
                print(f"  Waiting before retry...")
                time.sleep(2)  # Wait before retrying
            else:
                print(f"  ❌ Failed to process chunk {chunk_index+1} after {max_retries} attempts: {e}")
                return None
...
def process_document_to_graph(
    ...
    try:
        ...
        if not all_extracted_data:
            ...
            # THIS IS THE FIX: Define chunks before using it
